...
export async function execute(interaction) {
  try {
    ...
    const providerRole = guild.roles.cache.find(role => 
      role.name.toLowerCase().includes('provider') || 
    ...
    const vouchChannel = guild.channels.cache.find(channel => 
      channel.name.toLowerCase().includes('vouch') ||
      channel.name.toLowerCase().includes('order')
    );

    const casinoChannel = guild.channels.cache.find(channel => 
    ...
    const embed = new EmbedBuilder()
      ...
      .setDescription('Here are the detected/recommended IDs for your Railway environment variables:')
      .addFields(
        { 
    ...
    if (providerRole) {
      ...
      embed.addFields({
        ...
        value: 'Create a role named "Provider" or "Staff", then run `/setup` again.\n\n**Or manually add to Railway:**\n\`PROVIDER_ROLE_ID=your_role_id_here\`',
        inline: false
      });
    }
    ...
    if (adminRole) {
      ...
      embed.addFields({
        ...
        value: 'Create a role with Administrator permissions, then run `/setup` again.\n\n**Or manually add to Railway:**\n\`ADMIN_ROLE_ID=your_role_id_here\`',
        inline: false
      });
    }
    ...
    if (vouchChannel) {
      ...
      embed.addFields({
        ...
        value: 'Create a channel named "vouch" or "orders", then run `/setup` again.\n\n**Or manually add to Railway:**\n\`VOUCH_CHANNEL_ID=your_channel_id_here\`',
        inline: false
      });
    }
    ...
    if (casinoChannel) {
      ...
      embed.addFields({
        ...
        value: 'Create a channel named "casino" or "games", then run `/setup` again.\n\n**Or manually add to Railway:**\n\`CASINO_CHANNEL_ID=your_channel_id_here\`',
        inline: false
      });
    }
    ...
    if (logChannel) {
      ...
      embed.addFields({
        ...
        value: 'Create a channel named "logs" or "audit", then run `/setup` again.\n\n**Or manually add to Railway:**\n\`LOG_CHANNEL_ID=your_channel_id_here\`',
        inline: false
      });
    }
