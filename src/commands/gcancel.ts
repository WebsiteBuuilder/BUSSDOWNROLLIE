...
export const data = new SlashCommandBuilder()
  .setName('gcancel')
  .setDescription('Cancel an active giveaway and refund entrants')
...
export async function execute(interaction: ChatInputCommandInteraction) {
  ...
  if (
    ...
    await interaction.reply({ content: 'You do not have permission to cancel this giveaway.', ephemeral: true });
    return;
  }

  const success = await service.cancelGiveaway(targetId, interaction.user.id);
  if (!success) {
    await interaction.reply({ content: 'Unable to cancel giveaway.', ephemeral: true });
    return;
  }

  await scheduler.refreshGiveaway(targetId);
  await interaction.reply({ content: 'Giveaway cancelled and refunds issued.', ephemeral: true });
}
